...
@Entity
@Table(name = "chu")
public class Chu implements java.io.Serializable, AssociableEntity {
    ...
    @OneToMany(fetch = FetchType.LAZY, mappedBy = "chu")
    private Set<Lokality> lokalities = new HashSet<>(0);

    public Chu() {
    }

    public Chu(String meno) {
        this.meno = meno;
    }

    public Chu(String meno, boolean schvalene, Set<Lokality> lokalities) {
        this.meno = meno;
