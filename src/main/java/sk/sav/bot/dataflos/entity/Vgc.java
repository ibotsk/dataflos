...
@Entity
@Table(name = "vgc")
public class Vgc implements java.io.Serializable, AssociableEntity {
    ...
    @OneToMany(fetch = FetchType.LAZY, mappedBy = "vgc")
    private Set<Lokality> lokalities = new HashSet<>(0);

    public Vgc() {
    }

    public Vgc(String meno, boolean schvalene, Set<Lokality> lokalities) {
        this.meno = meno;
